...
pub async fn run_analysis() -> Result<()> {
    ...
    match regression_output {
        ...
        Err(e) => {
            error!(
                "Failed to execute python3: {}. Ensure python3 is in PATH.",
                e
            );
        }
    }
    ...
    match jupyter_output {
        ...
        Err(e) => {
            error!(
                "Failed to execute jupyter: {}. Ensure jupyter is installed.",
                e
            );
        }
    }
