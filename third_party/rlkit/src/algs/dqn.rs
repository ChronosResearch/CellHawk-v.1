...
use crate::{Action, EnvTrait, Status, Sample, QValue};
use super::*;
use crate::replay_buffer::ReplayBuffer;
use crate::network::NeuralNetwork;
use crate::utils::{decode_mixed_radix, dim_product, encode_mixed_radix, IntoF32};
use std::any::{Any};
    ...
    A: Copy + Clone + TryInto<usize> + TryFrom<usize> + 'static,
{
    ...
    pub fn new<Env>(env: &Env, buffer_capacity: usize, hidden_dims: &[usize], state_mode: DNQStateMode, device: &Device) -> Result<Self> 
    where
        Env: EnvTrait<S, A>,
        A: Copy + Clone + Product + TryInto<usize> + 'static,
    {
        ...
        // 创建Q网络和目标网络
        let q_network = NeuralNetwork::new(state_dim, hidden_dims, action_dim, device)?;
    ...
    fn update_q_network_batch(&mut self, batch_size: usize, gamma: f32) -> Result<()> {
        ...
        for sample in &samples {
            match self.state_mode {
                DNQStateMode::OneHot => {
                    ...
                    next_states.push(sample.next_state.to_one_hot_flat(&self.state_space, device)?);
                },
                DNQStateMode::Naive => {
                ...
                DNQStateMode::Normalized => {
                    states.push(sample.state.to_tensor_normalized(&self.state_space, device)?);
                    next_states.push(sample.next_state.to_tensor_normalized(&self.state_space, device)?);
                }
            }
        ...
        let next_state_tensor = Tensor::stack(&next_states, 0)?;
        ...
        // 创建奖励张量
        let reward_tensor = Tensor::from_vec(rewards, batch_size, device)?;
        
        // 创建done掩码张量
        let done_mask: Vec<f32> = dones.iter().map(|&done| if done { 0.0 } else { 1.0 }).collect();
        let done_mask_tensor = Tensor::from_vec(done_mask, batch_size, device)?;

        // 计算目标Q值
        let gamma = Tensor::new(gamma, device)?;
        let target_q_values = reward_tensor.add(
            &max_next_q_values.mul(&done_mask_tensor)?.broadcast_mul(&gamma)?
        )?;
        ...
        // 选择当前状态下对应动作的Q值
        let selected_q_values = current_q_values.gather(&action_indices.unsqueeze(1)?, 1)?
            .squeeze(1)?;
    ...
    fn get_q_values(&self, state: &Status<S>) -> Result<Vec<f32>> {
        let state_tensor = match self.state_mode {
            DNQStateMode::OneHot => state.to_one_hot_flat(&self.state_space, &self.q_network.device())?,
            DNQStateMode::Naive => state.to_tensor(&self.q_network.device())?,
            DNQStateMode::Normalized => state.to_tensor_normalized(&self.state_space, &self.q_network.device())?,
        }.unsqueeze(0)?;
        ...
        if q_values.shape() != &expected_shape {
            return Err(AlgorithmError::ModelUpdateFailed(
                format!(
                    "Q网络输出形状错误！期望 {:?}，实际 {:?}（动作总数: {}）",
                    expected_shape, q_values.shape(), self.action_dim
                )
            ));
        }
        
        // 转换为Vec<f32>
        let q_values_flat = q_values.squeeze(0)? // 移除批次维度，得到 (action_dim,)
        ...
        S: std::fmt::Debug + IntoF32 + Copy,
    {
        ...
        for _ in 0..args.max_steps {
            ...
            if self.step_count % 10000 == 0 {
                println!("当前步数: {}, 状态: {:?}, Q值: {:?}", self.step_count, state.value, q_values);
            }
            ...
            // 当缓冲区足够大时，进行批量更新
            if (self.replay_buffer.len() >= self.min_buffer_size) && (self.step_count % args.update_freq == 0) {
...
where
    Env: EnvTrait<S, A>,
    S: std::fmt::Debug + IntoF32 + Copy + TryInto<usize> + TryFrom<usize> + Product + 'static + 'static,
    A: Copy + Clone + TryInto<usize> + TryFrom<usize> + 'static,
{
