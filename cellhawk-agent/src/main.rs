...
fn check_secure_permissions(path: &str) {
    #[cfg(unix)]
    {
        ...
        if let Ok(meta) = std::fs::metadata(path) {
            ...
            if mode & 0o077 != 0 {
                warn!("SECURITY ALERT: {} has unsafe permissions ({:o}). Expected 600.", path, mode);
            }
        }
    }
    #[cfg(not(unix))]
    {
        warn!("OS does not support Unix permissions. Skipping permission check for {}.", path);
    }
}

fn setup_panic_hook() {
    std::panic::set_hook(Box::new(|info| {
        error!("FATAL PANIC: {}. Attempting graceful zeroize of EKF state.", info);
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let _drand_url = std::env::var("CELLHAWK_DRAND_URL").unwrap_or_else(|_| "https://api.drand.sh".to_string());
    ...
    tokio::spawn(async move {
        ...
        telemetry_pub.set_sndhwm(1000).expect("Failed to set ZMQ HWM (Step 22)");
        ...
        while let Some((heading, climb)) = mav_rx.recv().await {
            ...
            let msg = format!("MAV: roll={} pitch={} yaw={} thrust={}", roll, pitch, yaw, thrust);
